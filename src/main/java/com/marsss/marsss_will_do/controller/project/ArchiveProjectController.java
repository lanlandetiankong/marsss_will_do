...
public class ActivityProjectController extends MyBaseController{
    ...
    public MyBaseCommonResult doGetActivityProjectByTokenId(HttpServletRequest request) {
        MyBaseCommonResult result = new MyBaseCommonResult() ;
        try{
            UserAccount userAccount = super.handleCheckIsUserAccountToken(request,true);
            List<ActivityProjectBean> beanList = activityProjectService.doGetActivityProjectByUserAccount(request,userAccount);
            if(beanList == null){
                result.setCount(0);
            }   else {
